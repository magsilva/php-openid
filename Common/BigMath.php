...
class MathLibrary
{
    ...
    function rand($stop)
    {
        ...
        if (array_key_exists($rbytes, $duplicate_cache)) {
            ...
            if ($rbytes[0] == "\x00") {
                $nbytes = StringUtil::bytes($rbytes) - 1;
            } else {
                $nbytes = StringUtil::bytes($rbytes);
            }
        ...
        do {
            $bytes = "\x00" . CryptUtil::getBytes($nbytes);
 ...
 *
 * {@link MathLibrary_BcMathWrapper} wraps the functionality provided by the
 * BCMath extension.
 *
 * @access private
 * @package OpenID
 */
class MathLibrary_BcMathWrapper extends MathLibrary
{
    var $type = 'bcmath';
 ...
 *
 * {@link MathLibrary_GmpMathWrapper} wraps the functionality provided by the
 * GMP extension.
 *
 * @access private
 * @package OpenID
 */
class MathLibrary_GmpMathWrapper extends MathLibrary
{
    var $type = 'gmp';
...
class BigMath
{
	 ...
	 */
	function detectMathLibrary($exts)
	{
	    $loaded = false;
	...
	/**
	 * {@link getMathLib} checks for the presence of long number extension
	 * modules and returns an instance of {@link Auth_OpenID_MathWrapper}
	...
	function &getMathLib()
	{
	    ...
	    // works.
	    $ext = BigMath::detectMathLibrary(Auth_OpenID_math_extensions());
	    if ($ext === false) {
	        $tried = array();
	...
	function mathExtensions()
	{
	    ...
	    if (! defined('BIGMATH_BUGGY_GMP')) {
	        $result[] =
	            array('modules' => array('gmp', 'php_gmp'),
	                  'extension' => 'gmp',
	                  'class' => 'MathLibrary_GmpMathWrapper');
	    }
	
	    $result[] = array(
	                      'modules' => array('bcmath', 'php_bcmath'),
	                      'extension' => 'bcmath',
	                      'class' => 'MathLibrary_BcMathWrapper');
