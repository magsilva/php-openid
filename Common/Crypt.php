...
if (!defined('CRYPTOUTIL_RAND_SOURCE')) {
    /**
     * The filename for a source of random bytes. Define this yourself
     * if you have a different source of randomness.
     */
    define('CRYPTOUTIL_RAND_SOURCE', '/dev/urandom');
}
...
class CryptUtil
{
    ...
    function getBytes($num_bytes)
    {
        ...
        $bytes = '';
        if ($f === null) {
            if (CRYPTOUTIL_RAND_SOURCE === null) {
                $f = false;
            } else {
                $f = @fopen(CRYPTOUTIL_RAND_SOURCE, 'r');
                if ($f === false) {
                    $msg = 'Define CRYPTOUTIL_RAND_SOURCE as null to ' .
